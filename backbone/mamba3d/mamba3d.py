...
class Group(nn.Module):  # FPS + KNN
    ...
    def forward(self, xyz):
        ...
        # fps the centers out
        center, center_idx = fps(xyz, self.num_group, needidx=True) # B G 3
        ...
        return neighborhood, center, center_idx
...
class Mamba3DEncoder(nn.Module):
    ...
    def forward(self, center, x, pos):
        ...
        for _, block in enumerate(self.blocks):
              ...
              if _ in (3, 7, 11):
                block_features.append(x.clone().transpose(1, 2)[:, :, 1:].contiguous())
        return x, block_features


class Mamba3d_config:
...
class Mamba3D(nn.Module):
    ...
    def forward(self, pts):
        ...
        neighborhood, center, center_idx = self.group_divider(pts) # B G K 3
        ...
        # transformer
        x, block_features = self.blocks(center, x, pos) # enter transformer blocks
        # x = self.norm(x) # B G+1 C : B 65 384
        # concat_f = torch.cat([x[:, 0], x[:, 1:].max(1)[0] + x[:, 1:].mean(1)[0]], dim=-1)
        # ret = self.cls_head_finetune(concat_f)
        ret = None
        if self.dense:
            return ret, block_features, group_input_tokens, center, center_idx
        else:
            return ret
