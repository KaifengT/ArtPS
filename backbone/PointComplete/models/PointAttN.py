...
import math
from backbone.PointComplete.utils.model_utils import *
...
from .conv import EquivariantLayer
...
class Model(nn.Module):
    def __init__(self, args, num_parts=0, r1=2, r2=2):
        super(Model, self).__init__()


        self.encoder = PCT_encoder()

        self.refine = PCT_refine(ratio=r1)
        self.refine1 = PCT_refine(ratio=r2)
        
        self.pointnet = PointNet2()

        self.cls_1 = EquivariantLayer(128, 64, activation='relu', normalization='batch')
        self.cls_2 = EquivariantLayer(64, num_parts, activation=None, normalization=None)


    def forward(self, x, gt=None, gt_cls=None, kp=None, is_training=True):
        feat_g, coarse = self.encoder(x)

        if kp is not None:
            new_x = torch.cat([x, coarse, kp],dim=2)
        else:
            new_x = torch.cat([x, coarse],dim=2)
            
        new_x = gather_points(new_x, furthest_point_sample(new_x.transpose(1, 2).contiguous(), 512))

        fine, feat_fine, f00 = self.refine(None, new_x, feat_g)
        ...
        dense_seg_score = self.cls_2(self.cls_1(f11 + f00.repeat(1, 1, self.refine1.ratio))).transpose(1, 2).contiguous()
        
        # dense_feature = self.pointnet(fine1.clone().detach())
        # dense_seg_score = self.cls_2(self.cls_1(dense_feature)).transpose(1, 2).contiguous()
        ...
        if is_training:
            ...
            total_train_loss = loss1.mean() + loss2.mean() + loss3.mean() + cls_loss

            return fine, loss2, cls_loss, total_train_loss
        else:
            ...
            return {'out1': coarse, 'out2': fine1, 'cd_t_coarse': cd_t_coarse, 'cd_p_coarse': cd_p_coarse, 'cd_p': cd_p, 'cd_t': cd_t, 'seg_mask': seg_mask}
