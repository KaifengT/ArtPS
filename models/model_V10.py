...
class base_pem(nn.Module):
    def __init__(self, 
        ...
        if self.context_features == 1024:
            self.global_feature_layer = nn.Sequential(
                ResMLPBlock(192, 256),
                ResMLPBlock(256, 512),
                ResMLPBlock(512, 1024),
            )
            
            
        if not 'revolute' in self.joint_type:
            self.joint_loc_layer_1.requires_grad_(False)
            self.joint_loc_layer_2.requires_grad_(False)
            self.joint_loc_layer_3.requires_grad_(False)
            self.joint_loc_att_1.requires_grad_(False)
            self.joint_loc_att_2.requires_grad_(False)
    ...
    def forward(self, x, **kwargs):
        ...
        elif self.context_features == 1024:
            feature_for_glow = self.global_feature_layer(dense_feature.mean(dim=-1))
        ...
        if self.training:
            ...
            if self.reverse_on_training:
                out = self.flow_forward(feature_for_glow, x, dense_seg_score.clone().detach(), kwargs['num_sample'], kwargs['connection_matrix'])
                rdata.update(out)
                            
            
        else:
            
            out = self.flow_forward(feature_for_glow, x, dense_seg_score, kwargs['num_sample'], kwargs['connection_matrix'])
    ...
    def flow_forward(self, condition, cloud, dense_seg_score, top_k_sample, connection_matrix):
        rdata = {}
        NP = self.num_parts
        B = condition.shape[0]
        ...
        # beta = pred_token[:, :num_sample, -10:]
        # canonical_global_kp = self.get_norm_keypoints(beta.reshape(-1, self.num_basis)).reshape(B, top_k_sample, NP, -1, 3)
        # joint_loc, joint_axis = self.get_norm_joint_params(beta.reshape(-1, self.num_basis))
    ...
    @staticmethod
    def decode_articulation(base_pose, norm_joint_loc, norm_joint_axis, joint_state, with_base=True, joint_type=[], connection_matrix=None):
        '''
        Args:
            ...
            connection_matrix (torch.Tensor) (P, P): connection matrix of the articulation, where connection_matrix[i, j] = 1 if part i is connected to part j, else 0. The order of parts should be the same as the order of joint parameters. If None, it is assumed that all parts are connected to the base part.
        ...
        part_pose_dict = {0: base_pose if with_base else torch.eye(4, device=base_pose.device).expand_as(base_pose)}
        
        def get_parent_pose(pid):
            if pid in part_pose_dict:
                return part_pose_dict[pid]
            
            if connection_matrix is None:
                parent_id = 0
            else:
                parent_idx = torch.where(connection_matrix[pid] == 1)[0]
                parent_id = parent_idx[0].item() if len(parent_idx) > 0 else 0
                
            p_pose = get_parent_pose(parent_id)
            ...
            part_pose = p_pose @ tmat2 @ aug_matrix @ tmat1
            part_pose_dict[pid] = part_pose
            return part_pose

        part_pose_list = []
        for pid in range(1, J + 1):
            pose = get_parent_pose(pid)
            part_pose_list.append(pose[..., None, :, :])
    ...
    def tokenlize_articulation_2(self, rt, beta, joint_state, scale, **kwargs):
        ...
        token = torch.cat([base_token, beta, joint_state_token, scale / 10.0], dim=-1) # B 9 + 1 * (P-1) + 1
    ...
    def DEtokenlize_articulation_2(self, token, **kwargs):
        ...
        # NOTE: scale 'scale' to real value
        return base_pose, beta, joint_state, scale * 10.0
