...
# NOTE: This version includes bounding box prediction head
...
class Decoder(nn.Module):
    def __init__(
        ...
        self.inscode_proj = nn.Sequential(
            ...
            # 输出层：生成 6 * catcode_size（如 γ1, β1, γ2, β2, γ3, β3）
...
class SDFDecoder(nn.Module):
    def __init__(self, num_catcodes, catcode_size, inscode_size, num_parts, ):
        super().__init__()
        ...
        self.joint_net = JointBranch(self.catcode_size, [64, 64, self.num_joints * 6],
        ...
        self.bbox_head = nn.Sequential(
            ResMLPBlock(self.catcode_size, 256, hidden_dim=512, dropout=0.1),
            ResMLPBlock(256, self.num_parts * 6, hidden_dim=512, dropout=0.1)
        )
    ...
    def forward(self, inscode, query_point, chunk_size=1):
        ...
        norm_joint_loc, norm_joint_axis = self._forward_joint(code)
    ...
    def forward_joint(self, inscode:torch.Tensor):
        ...
        norm_joint_loc, norm_joint_axis = self._forward_joint(feat)
        return norm_joint_loc, norm_joint_axis
...
if __name__ == '__main__':
    ...
    sdfdecoder = SDFDecoder(num_catcodes=32, catcode_size=256, inscode_size=64, num_parts=2)
    
    inscode = torch.randn(2, 64)
