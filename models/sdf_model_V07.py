...
class SDFDecoder(nn.Module):
    ...
    def forward(self, inscode, query_point, chunk_size=1):
        ...
        Return:
            dict:
                'sdf': sdf,  # [B, N, 1]
                'norm_joint_loc': norm_joint_loc,  # [B, num_joints, 3]
                'norm_joint_axis': norm_joint_axis,  # [B, num_joints, 3]
        '''
        
        sdf, feat = self.decoder(query_point, inscode)
        
        norm_joint_loc, norm_joint_axis = self._forward_joint(self.decoder._proj_code_for_joint(feat))
...
if __name__ == '__main__':
    ...
    sdf_values = sdfdecoder(inscode, query_point)
    loss = sdfdecoder.catcode_regularization(l2_weight=1e-4, diversity_weight=1e-3)
    print("SDF Values shape:", sdf_values.shape)  # Expected: (2,
